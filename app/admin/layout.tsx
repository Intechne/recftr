...
["/admin/programlar","Programlar","programs",["admin","editor"]],
["/admin/etkinlikler","Etkinlikler","events",["admin","editor"]],
["/admin/haberler","Haberler & Duyurular","news",["admin","editor"]],
["/admin/dokumanlar","Public Dokümanlar","documents",["admin","editor"]],
...
["/admin/takim-belgeleri","Takım Belgeleri","portal-docs",["admin","approvals","technical"]],
["/admin/odemeler","Ödemeler","payments",["admin","approvals"]],
["/admin/iletisim","İletişim Kutusu","inbox",["admin","editor","approvals"]],
["/admin/ekip","Ekip & Yetkiler","users",["admin"]],
["/admin/ayarlar","Site Ayarları","settings",["admin","editor"]],
["/admin/audit","İşlem Geçmişi","audit",["admin"]],
["/admin/sistem","Sistem Tanılama","security",["admin"]],
];
  ...
  return <div className="min-h-screen bg-paper lg:grid lg:grid-cols-[260px_minmax(0,1fr)] 2xl:grid-cols-[285px_minmax(0,1fr)]">
    ...
    <div className="min-w-0">
      <header className="sticky top-0 z-40 flex min-h-16 items-center justify-between gap-3 border-b border-ink/10 bg-white/95 px-4 py-3 backdrop-blur sm:px-5 lg:px-7 2xl:min-h-[72px] 2xl:px-10">
        <div className="flex min-w-0 items-center gap-3"><button type="button" onClick={()=>setOpen(true)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border-2 border-ink lg:hidden" aria-label="CMS menüsünü aç"><FigmaIcon name="settings" className="h-5 w-5"/></button><span className="truncate font-display text-[12px] font-semibold tracking-[1px] text-ink sm:text-[13px] 2xl:text-[14px]">YÖNETİM PANELİ</span></div>
        <div className="flex shrink-0 items-center gap-2 sm:gap-4 text-[11px] font-semibold text-ink/55 sm:text-[12px]"><Link href="/" className="hidden items-center gap-1.5 hover:text-cyan-deep sm:flex"><FigmaIcon name="home" className="h-4 w-4"/> Site</Link><span className="hidden max-w-[220px] truncate md:inline">{me?.name||"Yönetici"} · {me?.role||""}</span><Logout/></div>
      </header>
