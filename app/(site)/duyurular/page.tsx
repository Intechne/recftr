...
export default function DuyurularPage(){
 ...
 <div className="safe-x mx-auto max-w-7xl lg:px-10">
  ...
  {featured&&<Link href={`/duyurular/${featured.slug}`} className="mt-8 flex flex-col overflow-hidden rounded-xl border-2 border-ink bg-white shadow-plate shadow-cyan-brand transition-transform hover:-translate-y-1 lg:flex-row">
    ...
    <div className="flex-1 p-5 sm:p-7 lg:p-9"><span className="rounded bg-cyan-brand px-2.5 py-1 font-display text-[11px] font-bold text-ink">{featured.tag} · ÖNE ÇIKAN</span><h2 className="mt-3.5 font-display text-[24px] font-bold leading-tight text-ink lg:text-[28px]">{featured.title}</h2><p className="mt-3 text-[15px] leading-relaxed text-ink/60">{featured.excerpt}</p><p className="mt-4 font-display text-[12px] font-medium tracking-[1px] text-cyan-deep">{fmt(featured.date).toUpperCase()} · DEVAMINI OKU →</p></div>
  </Link>}
  <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 2xl:gap-5">{rest.map(n=><Link key={n.slug} href={`/duyurular/${n.slug}`} className="flex h-full flex-col overflow-hidden rounded-xl border-[1.5px] border-ink/20 bg-white transition-shadow hover:shadow-lg">
