...
export default function KayitPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/pricing").then(r=>r.ok?r.json():null).then(x=>x&&setPricing(x)).catch(()=>{});
  }, []);

  useEffect(()=>{
    if(!form.city){setDistricts([]);return;}
    let live=true;setDistrictLoading(true);setDistricts([]);
    fetch(`/api/locations?province=${encodeURIComponent(form.city)}`,{cache:"force-cache"})
      .then(async r=>{const j=await r.json().catch(()=>({districts:[]}));return r.ok&&Array.isArray(j.districts)?j.districts:[]})
      .then(rows=>{if(live)setDistricts(rows)})
  ...
  function validateCurrentStep(){
    if(step===0 && !prog) return "Lütfen bir program seçin.";
    if(step===1){
      if(!form.team.trim()) return "Takım adı zorunludur.";
      if(!form.org.trim()) return "Okul / kurum bilgisi zorunludur.";
      if(!form.city) return "Lütfen il seçin.";
      if(!form.district) return "Lütfen ilçe seçin.";
      if(!form.num.trim()) return "Takım numarası zorunludur.";
      if(!/^[A-Z0-9]{2,10}$/.test(form.num)) return "Takım numarası yalnızca harf ve rakamlardan oluşmalıdır.";
    }
    if(step===2){
      if(!form.mentor.trim()) return "Mentor ad soyad zorunludur.";
      if(!/^\S+@\S+\.\S+$/.test(form.email)) return "Geçerli bir e-posta adresi girin.";
      if(!form.phone.trim()) return "GSM numarası zorunludur.";
    }
    return "";
  }

  function nextStep(){const e=validateCurrentStep();if(e){setErr(e);return;}setErr("");setStep(s=>Math.min(3,s+1));}
        ...
        <ol className="flex flex-wrap items-center gap-3">
          {steps.map((s, i) => (
            <li key={s} className="flex items-center gap-3">
              <button onClick={() => i < step && setStep(i)} className={`diamond flex h-9 w-9 items-center justify-center rounded-md border-2 border-ink font-display text-[15px] font-bold ${i <= step ? "bg-cyan-brand" : "bg-white"}`} aria-current={i === step ? "step" : undefined}><span>{i + 1}</span></button>
